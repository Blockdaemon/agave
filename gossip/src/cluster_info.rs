...
impl ClusterInfo {
    ...
    fn process_packets(
        ...
        for (from_addr, protocol) in packets {
            ...
            // --- Deferred Verification and Processing --- START
            // Packet passed filter, verify BEFORE adding to batch
            if protocol.par_verify() {
                self.stats.packets_received_verified_count.add_relaxed(1); // Increment stat here

                // Packet is verified, proceed with original processing logic
                match protocol {
                ...
                debug!(
                    "Deferred verification failed for {:?} from {}", // Use Debug format
                    filterable_protocol, from_addr
                );
                // Optionally increment a new stat here if needed
            }
            // --- Deferred Verification and Processing --- END
        }
